...
class SubscriptionProfile extends React.Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.basic !== prevState.basic) {
      ...
      const { customer_id, packages } = basic;
      if (packages)
        return ({ webview: (customer_id ? false: true), pkgs: packages }); // <- this is setState equivalent
      else 
        return ({ webview: (customer_id ? false: true)}); // <- this is setState equivalent
    }
    return null
  }
  navigateTo = page => {
